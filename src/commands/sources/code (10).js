...
class YouTubeCommand extends Command {
  constructor() {
    super({
      ...
      usage: 'youtube <song name or URL>',
      aliases: ['yt'],
      category: 'music',
      cooldown: 2,
      voiceRequired: true,
  ...
  async execute({ message, args, client, musicManager }) {
    ...
    try {
      ...
      if (!result || !result.tracks || !result.tracks.length) {
        const reply = embedManager.error('No Results', `No results found for: \`${query}\` on ${platformDisplayName}`);
        return searchingMsg.edit({ embeds: [reply] });
      }
      ...
      if (isPlayerPlaying) {
        if (isPlaylist) {
          ...
          collector.on('collect', async (interaction) => {
            ...
            // Assuming localPlayer.queue is array-like or PlayerManager makes it so.
            // Or this should be localPlayer.queue.tracks[targetIndex] if using Kazagumo Queue directly
            ...
            try {
                localPlayer.queue.remove(targetIndex); // Kazagumo remove by index
                localPlayer.queue.add(trackToMove, 0); // Kazagumo add to front
